...
def serve_mcp_server_card():
    ...
    return {
        ...
        "tools": [
            ...
            {"name": "get_wallet_trust_score",    "description": "GET /wallet/score/{address} — AgentTrust Wallet Trust Score (0–100). The only open trust-scoring system built natively for XRPL wallets. Combines 7 independent signals: account age, XRP balance, on-chain activity, domain verification status, NFTs held, AgentTrust escrow completion rate (pass/fail history from real jobs), and peer ratings left by counterparties after completed escrows. Returns a full score breakdown by signal so agents can reason about why a wallet scores high or low. Agents should check this before hiring or accepting a job — a score below 30 is low-trust, 30–60 is moderate, 60+ is established. Free to query for any XRPL address."},
